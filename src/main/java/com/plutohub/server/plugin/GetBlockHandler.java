...
public class GetBlockHandler extends PlutoHttpHandler {
  public GetBlockHandler(final BackendPlugin backend) {
    super(backend);
  }
  ...
  protected void execute(final HttpServerExchange exchange, final ServerSecurityUser user) throws Exception {
    ...
    database.begin();
    try {
      final IndexCursor cursor = database.lookupByKey(BitcoinSchema.VERTEX_BLOCK, "hash", idParam.getFirst());
      if (cursor.hasNext()) {
        ...
        exchange.getResponseSender().send("{ \"error\" : \"block not found\"}");
      }
    } finally {
